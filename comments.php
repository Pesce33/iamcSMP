...
<section id="respond" class="respond-form">
	...
	<h3 id="comment-form-title" class="h2"><?php comment_form_title( __( 'Write comment', 'bonestheme' ), __( 'Comment for %s', 'bonestheme' )); ?></h3>
	...
	<?php if ( get_option('comment_registration') && !is_user_logged_in() ) : ?>
		<div class="alert alert-help">
			<p><?php printf( __( 'You have to be %1$s logged %2$s for adding comment.', 'bonestheme' ), '<a href="<?php echo wp_login_url( get_permalink() ); ?>">', '</a>' ); ?></p>
		</div>
	<?php else : ?>
	...
	<p class="comments-logged-in-as"><?php _e( 'Logged as', 'bonestheme' ); ?> <a href="<?php echo get_option( 'siteurl' ); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php _e( 'Logout', 'bonestheme' ); ?>"><?php _e( 'Logout', 'bonestheme' ); ?> <?php _e( '&raquo;', 'bonestheme' ); ?></a></p>
	...
	<p><textarea name="comment" id="comment" placeholder="<?php _e( 'Write your comment here...', 'bonestheme' ); ?>" tabindex="4"></textarea></p>

	<p>
		<input name="submit" type="submit" id="submit" class="button" tabindex="5" value="<?php _e( 'Send', 'bonestheme' ); ?>" />
